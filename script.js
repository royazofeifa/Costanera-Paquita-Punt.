...
function validarDatoDiscursante(id) {
  const valor = limpiarTexto($(id).value);
  let mensaje = "";
  if (!valor) mensaje = id === "cancion-discursante" ? "Por favor indique el número de la canción." : "Por favor indique el nombre de su congregación.";
  else if (id === "cancion-discursante" && !/^\d*[1-9]\d*$/.test(valor)) mensaje = "Por favor indique un número entero mayor que cero.";
  else if (id === "congregacion-discursante" && valor.length > LIMITES.congregacion) mensaje = "La congregación debe tener un máximo de 180 caracteres.";
  if (mensaje) mostrarError(id, mensaje); else limpiarError(id);
  return !mensaje;
}
function obtenerNombre() {
  return datosDiscurso?.orador || limpiarTexto($("orador-discursante").value);
}
function validarNombreDiscursante() {
  if (datosDiscurso?.orador) return true;
  const nombre = obtenerNombre();
  let mensaje = "";
  if (!nombre) mensaje = "Por favor indique el nombre del discursante.";
  else if (nombre.length > LIMITES.orador) mensaje = "El nombre debe tener un máximo de 150 caracteres.";
  if (mensaje) mostrarError("orador-discursante", mensaje); else limpiarError("orador-discursante");
  return !mensaje;
}
function validarConfirmacion() {
  if (!datosDiscurso) return false;
  let valido = validarTituloDiscursante();
  if (!validarNombreDiscursante()) valido = false;
  ["cancion-discursante", "congregacion-discursante"].forEach((id) => {
    if (!validarDatoDiscursante(id)) valido = false;
  });
...
function iniciarConfirmacion(parametros) {
  ...
  valido = valido && fechaValida(datos.fecha) && numeroOpcionalValido(datos.bosquejo) && numeroOpcionalValido(datos.cancion);
  if (!valido) {
  ...
  datosDiscurso = datos;
  const necesitaNombre = !datos.orador;
  $("orador-discursante-group").hidden = !necesitaNombre;
  $("orador-discursante").disabled = !necesitaNombre;
  $("orador-discursante").required = necesitaNombre;
  $("speaker-heading").hidden = necesitaNombre;
  $("speaker-card").hidden = necesitaNombre && !datos.congregacion;
  $("orador-discursante").addEventListener("input", () => {
    if (contenedorError("orador-discursante").classList.contains("field-error")) validarNombreDiscursante();
  });
  $("congregacion-info-label").hidden = !datos.congregacion;
  $("info-congregacion").hidden = !datos.congregacion;
  ["cancion-discursante", "congregacion-discursante"].forEach((id) => $(id).addEventListener("input", () => {
    if (contenedorError(id).classList.contains("field-error")) validarDatoDiscursante(id);
  }));
